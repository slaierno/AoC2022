...
struct FileSystem {
    ...
    template <class Compare = std::less<Folder*>>
    auto rfind(const auto& filter) {
        std::set<Folder*, Compare> rfind_result;
...
int main(int argc, char* argv[]) {
    ...
    fs.cdroot();
    auto top_folders =
        fs.rfind([](auto cwd) { return cwd->get_total_size() <= 100000; });

    std::cout << accumulate(top_folders, 0ULL, [](auto s, auto f) {
        return s + f->get_total_size();
    }) << std::endl;
    return 0;
}
