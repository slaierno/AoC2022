...
class RPS {
   ...
   public:
    ...
    constexpr auto operator<=>(const RPS& rhs) const {
        if (m_rps == rhs.m_rps)
            return 0;
        else if ((m_rps + 1) % 3 == rhs.m_rps)
            return -1;
        else
            return 1;
    }
    constexpr auto score() const { return m_rps + 1; }
    constexpr auto get_winlose(int winlose) const {
        return RPS((m_rps + winlose + 3) % 3);
    }
    constexpr auto get_winlose(char c) const {
        return get_winlose(c - 'X' - 1);
    }
};
